  ...
  begin
    ...
    begin
      ...
      adotblUsers['EMail'] := edtEmailPre.Text + '@' + edtEmailSuf.Text +
        cbEmail.Text;
      // Repeatedly present input box until a valid school year is entered
      // This completes levels that are below the user's current level of knowledge
      repeat
        repeat
          vStr := InputBox('User Info',
            'Enter the school year the user is currently attending (0 for reception, max year 5):',
            '0');
        until TryStrToInt(vStr, vInt);
      until (vInt >= 0) and (vInt <= 5);
      // Set user progress to default (0 for each level)
      for i := 1 to 12 do
        adotblUsers['Level' + inttostr(i)] := 0;
      // Depending on user input, mark levels below their current knowledge as complete
      for i := 1 to (vInt * 2) do
        adotblUsers['Level' + inttostr(i)] := LexiconSize[i];
